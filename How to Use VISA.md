...
VISAは、この複雑な通信方法の違いを気にせず、**同じ操作方法API**でどのメーカーの、どの接続方式の機器でも統一的に操作できるようにします。NI（National Instruments）が開発し、現在は多くの計測器メーカーが採用している業界標準です。
...
| **GPIB** | **計測器の世界で長く使われてきた太いケーブル接続規格**（HPが開発）。古めの機器で使用されています。 |
