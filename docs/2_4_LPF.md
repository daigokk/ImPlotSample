...
- レポートタイトル(自動計測実習 ローパスフィルタ)、学籍番号、氏名、図の説明、図番号を記載してください。


1. カットオフ周波数100Hzとして、一から三次での10Hzから10000Hzまでの周波数特性をボード線図(Gainと位相 $\theta$ )で示せ。
        ...
        if (ImGui::Button("Run")) {
            ...
            for (int j = 0; j < N_FREQS; j++) {
                ...
                for (int i = 0; i < N_TH; i++) {
                    ...
                    gains[i][j] = 20 * log10(pow(pow(x, 2) + pow(y, 2), 0.5));
