...
1. モンティ・ホール問題のシミュレーターを作る。
   - Lv. 10、60分
   - 【問題文】 3つのドア（A, B, C）があり、そのうちの1つだけが「当たり」で、残りの2つは「ハズレ」です。 プレイヤーがドアを1つ選んだ後、司会者は残りのドアのうち「ハズレ」のドアを1つ開けて見せます。 ここでプレイヤーは、最初に選んだドアのままにするか、残っているもう一つのドアに変更するかを選ぶことができます。このゲームを数千回～数万回シミュレーションし、「ドアを変えない場合」と「ドアを変える場合」のそれぞれの勝率（当たりを引く確率）を計算して表示するプログラムを作成してください。
   - 【要件】
