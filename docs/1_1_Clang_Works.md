# 1.1. 課題

1. C言語の基本構文: 標準出力(コンソール画面:黒いWindow)に`Hello world`を表示する。
   - Lv. 1、15分
   - 評価基準: 正しく「Hello world」が表示される。
     ```
     Hello world
     ```
1. scanfとprintf: 標準入力(キーボード)から入力された実数を、標準出力に表示する。
   - Lv. 1、15分
   - 評価基準: 入力された実数が正しく表示される。
   - Visual Studioではデフォルトではscanfが使えません。`scanf_s`を使うか、プログラムの最初の行に`#define _CRT_SECURE_NO_WARNINGS`を挿入してください。
     ```
     How tall are you?> 185
     Tall> 185.000000
     ```
1. if文: 標準入力から入力された整数の、奇数または偶数を判定する。判定結果は標準出力に表示する。
   - Lv. 2、15分
   - 評価基準: 奇数・偶数の判定が正しく行われる。
     ```
     Input> 5
     Output> Odd number
     ```
1. for文: for文のネスト(入れ子構造)を用いて以下の文字列を出力する。
   - Lv. 3、30分
   - 評価基準: 指定された文字列が正しく整形されて出力される。
   - 加点例: 文字列の並びを横方向、または逆方向にする。
     ```
     A G M S Y _ e k q w 
     B H N T Z ` f l r x 
     C I O U [ a g m s y 
     D J P V \ b h n t z 
     E K Q W ] c i o u { 
     F L R X ^ d j p v | 
     ```
1. for文とif文と文字配列: 標準入力から入力された文字列を大文字に変換して出力する。
   - Lv. 4、30分
   - 評価基準: 大文字で出力される。加点例: 小文字は大文字、大文字は小文字に変換する。
   - Visual Studioではデフォルトではscanfが使えません。`scanf_s`を使うか、プログラムの最初の行に`#define _CRT_SECURE_NO_WARNINGS`を挿入してください。
     ```
     Input> Ptu
     Output> PTU
     ```
1. 配列と関数: 配列に記録されている数値の最小値と最大値を画面に表示して終了する。関数minimaxの定義部分を作成する。
   - Lv. 4、45分
   - 評価基準: minimax関数が正しく定義され、結果が正しく表示される。加点例: 平均 $\bar{x}$ や不偏標準偏差 $\sigma = \sqrt{\frac{1}{N-1} \sum_{i=1}^{N} (x_i - \bar{x})^2}$ を表示する。
     ```c
     #include <stdio.h>
     #include <math.h> // 標準偏差を求める際に必要なルートを求めるsqrt関数を使う際に必要です
     void minimax(int length, double *arr, double *minimum, double *maximum);
     int main(void) {
         double arr[] = { 1,5,4,3,2 }, minimum, maximum;
         minimax(5, arr, &minimum, &maximum);
         printf("%f, %f\n", minimum, maximum);
         return 0;
     }
     
     void minimax(int length, double *arr, double *minimum, double *maximum) {
         // ここに適切なコードを記入

         
     }
     ```
1. 配列と関数: 以下のプログラムは配列に記録されている数値を昇順に並び替えて画面に表示して終了する。関数sortの定義部分を作成する。
   - Lv. 5、60分
   - 評価基準: sort関数が正しく定義され、配列が昇順に並ぶ。
     ```c
     #include <stdio.h>
     void sort(int length, double *arr);
     int main(void) {
         double arr[] = { 1,5,4,3,2 };
         sort(5, arr);
         for (int i = 0; i < 5; i++) {
             printf("%f\n", arr[i]);
         }
         return 0;
     }
     
     void sort(int length, double *arr) {
         // ここに適切なコードを記入
         
         
     }
     ```
1. for文と数学関数(sin等): 標準入力から得た振幅、及び位相を用いて、100kHzの正弦波5周期分の時間と値を`,`区切りの実数で標準出力に表示する。ただし、サンプリング間隔`dt`は1e-8sとする。
   - Lv. 5、30分
   - 評価基準: 100kHzの波形が正しく生成され、時間・値が正確に出力される
   - Visual Studioではデフォルトではscanfが使えません。`scanf_s`を使うか、プログラムの最初の行に`#define _CRT_SECURE_NO_WARNINGS`を挿入してください。
     ```cpp
      #include <stdio.h>
      #include <math.h>
      #define PI acos(-1)
      int main() {
          double amp, phase_deg;
          double omega = 2 * PI * 100e3;
          scanf("%lf %lf", &amp, &phase_deg);
          double phase_rad = phase_deg / 180.0 * PI;
          /*** ここに適切なコードを記入 ***/
          
          
          /*** ここまで ***/
          return 0;
      }
     ```
1. 測定データの保存(fopen, fprintf, fclose): 上記の結果を`data.csv`ファイルに出力する。
   - Lv. 6、15分
   - 評価基準: data.csvが正しく作成され、内容が正確に記録されている。
1. 外部ライブラリを用いたデータの可視化: `data.csv`の値を[ImPlot](https://github.com/epezent/implot)を用いてグラフにして表示する。
   - Lv. 100、200分
   - 評価基準: グラフが正しく表示され、ラベル・軸が適切に設定されている。
   - [これ](https://github.com/daigokk/ImPlotSample/tree/master)を使えば簡単(Lv. 50くらい)になります。
     
     ![Raw waveform](./images/Clang_10.png)
1. モンテカルロ法を用いた円周率の導出
   - Lv. 8、60分
   - 評価基準: 円周率を6桁(3.14159)以上表示する。検算には`math.h`を用いてもよい。加点例: マクローリン展開を用いた円周率の導出。
   - 手順:
     - -1～1までの値を取る二つの乱数`x`,`y`を求める。
     - 得られた`xy`座標上の点が、原点を中心とする半径1mの円の中にあるかを判定する。
     - 上記を標準入力から得た`n`の回数実施し半径1mの円(面積 $\pi \rm m^2$ )の中にある点の数`k`回と一辺が2mの正方形(面積 $4 \rm m^2$ )の中にある点の数`n`の比から得られる式 $\pi/4=k/n$ を用いて円周率 $\pi$ を求める。

     ![PI](./images/Clang_11.png)

1. ニュートン・ラプソン法を用いた $\sqrt{2}$ の導出
   - Lv. 7、30分、評価基準
   - 評価基準: 6桁(1.41421)以上表示する。検算には`math.h`を用いてもよい。加点例: 任意の実数のルートを求める。
   -  $x_{i+1}=x_i - \frac{f(x_i)}{f'(x_i)}$


1. モンテカルロ法を用いてモンティ・ホール問題のシミュレーターを作る。
   - Lv. 10、60分
   - 【問題文】
      - 3つのドア（A, B, C）があり、そのうちの1つだけが「当たり」で、残りの2つは「ハズレ」です。 プレイヤーがドアを1つ選んだ後、司会者は残りのドアのうち「ハズレ」のドアを1つ開けて見せます。 ここでプレイヤーは、最初に選んだドアのままにするか、残っているもう一つのドアに変更するかを選ぶことができます。このゲームを数千回～数万回シミュレーションし、「ドアを変えない場合」と「ドアを変える場合」のそれぞれの勝率（当たりを引く確率）を計算して表示するプログラムを作成してください。
   - 【要件】
      - 試行回数: 定数または変数で設定できるようにする（例：10,000回）。
      - 乱数: 当たりのドア、プレイヤーが最初に選ぶドアは毎回ランダムに決定する。
      - 司会者の行動: 司会者は「プレイヤーが選んでいない」かつ「ハズレ」のドアを開けなければならない。
      - 出力: 最終的に「変えない場合の勝率」と「変える場合の勝率」を％で表示する。
   - 【ヒント】
      - 乱数を使うには <stdlib.h> の rand() と <time.h> の time() を使います。
      - ドアを 0, 1, 2 の整数で表すと扱いやすいです。
 

### 参考資料
- [Visual Studioの使い方](./1_3_VisualStudio.md)
- [C言語 on Colab](https://colab.research.google.com/drive/1fewkHpqIm40EXWWdZ9eu6EUIN9MWQNO4)
- [C言語 課題解決チートシート](./1_2_Clang_CheatSheet.md)
- [解答](./1_4_Clang_Answers.md)
